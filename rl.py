...
class PacManEnv(gym.Env):
    def __init__(self, game, initial_pellets, x, y, width, height):
    ...
    def _get_obs(self):
        img = capture_game(self.x, self.y, self.width, self.height)
        # pprint(img)
        return img / 255.0
        # pellet_surrounding = []
...
if __name__ == "__main__":
    ...
    pos = win32gui.GetWindowRect(pygame.display.get_wm_info()['window'])

    pellet_list = [(pellet.position.x, pellet.position.y) for pellet in game.pellets.pelletList]
    env = PacManEnv(game, pellet_list, pos[0], pos[1], width, height)

    # train_policy_dqn(env)
    train_policy_ppo(env, pos[0], pos[1], width, height)